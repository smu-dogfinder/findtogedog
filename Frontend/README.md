...
## 프로젝트 구조
src/ ├── api/ # 백엔드 서버와의 통신(API 호출) 함수 관리 ├── components/ # 재사용 가능한 UI 컴포넌트 모음 ├── contexts/ # 전역 상태 관리 (예: 사용자 인증, AI 검색 상태) ├── pages/ # 라우팅 및 핵심 비즈니스 로직을 포함하는 페이지 컴포넌트 ├── styles/ # 전역 스타일 및 유틸리티 CSS 파일 └── assets/ # 이미지, 폰트 등 정적 리소스
